...
const catalog: ItemDef[] = [
  ...
  },
  {
    id: "4",
    name: "E-Reader",
    description: '6" glare-free, 8GB',
    unit_price: 79.99,
  },
  {
    id: "5",
    name: "Wireless Mouse",
    description: "Ergonomic, USB-C",
    unit_price: 24.99,
  },
];
...
const Page: FC = () => {
  ...
  const isDateValid = useMemo<boolean>(() => {
    ...
    const [year, month, day] = receiptDate
      .split("-")
      .map(Number);
    ...
    (id: string, qty: number) => {
      setItemMap((prev) => {
        ...
        m.set(id, {
          ...
          quantity: newQty,
          total_price: parseFloat(
            (newQty * ex.unit_price).toFixed(2)
          ),
        });
        return m;
      });
  ...
  >(async () => {
    ...
    const signature_hash = hashArray
      .map((b) =>
      ...
      <Card>
        ...
        <CardFooter>
          <Button
            ...
            onClick={() =>
              router.push(
                `/purchasing/suppliers/${supplier.slug}/configure`
              )
            }
          >
            Configure
...
<DialogDescription>
  {`On ${receiptDate}, Receipt No. ${receiptNumber}, a total of ${items.length} item(s) were purchased: ` +
    items.map(it => `${it.name} x ${it.quantity} ($${it.total_price.toFixed(2)})`).join(", ") + "."}
</DialogDescription>
