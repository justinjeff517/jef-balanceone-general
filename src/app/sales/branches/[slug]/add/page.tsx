...
export default function Page() {
    ...
    setItemMap(m => {
      ...
      if (ex) {
        ...
        next.set(def.id, {
          ...
          unit_price: def.unit_price, quantity:1, total_price: +def.unit_price.toFixed(2)
        });
      }
      return next;
    });
  ...
  const handleSubmit = async () => {
    ...
    const summaryText       = `Sale of ${items.length} item(s) totaling $${total.toFixed(2)} at ${branch.name} on ${receiptDate}.`;
    const itemsSummary      = items.map(it => `• ${it.name} x${it.quantity} ($${it.total_price.toFixed(2)})`).join("\n");
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        <div>
          <label htmlFor="receiptDate" className="block mb-1">Receipt Date</label>
          <Input
            id="receiptDate" placeholder="YYYY-MM-DD"
            value={receiptDate}
        ...
        <div>
          <label htmlFor="receiptNumber" className="block mb-1">Receipt #</label>
          <Input
            id="receiptNumber" placeholder="12345"
            value={receiptNumber}
