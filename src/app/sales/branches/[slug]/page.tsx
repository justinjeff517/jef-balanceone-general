...
type Branch = { branch_name: string; branch_slug: string; branch_tin: string };
const dummyBranches: Branch[] = [
  { branch_name: "Central Branch", branch_slug: "central-branch", branch_tin: "987-654-321" },
  { branch_name: "North Branch",   branch_slug: "north-branch",   branch_tin: "987-654-322" },
  { branch_name: "East Branch",    branch_slug: "east-branch",    branch_tin: "987-654-323" },
];
...
export default function Page() {
  ...
  const branch = useMemo(
    () =>
      dummyBranches.find((b) => b.branch_slug === slug) || { branch_name: slug, branch_slug: slug, branch_tin: "" },
    [slug]
  );
  ...
  const addItem = useCallback((def: ItemDef) => {
    setItemMap((prev) => {
      ...
      if (ex) {
        ...
        m.set(def.id, { ...ex, quantity: qty, total_price: +(qty * def.unit_price).toFixed(2) });
      } else {
        m.set(def.id, {
          id: def.id,
          name: def.name,
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <h1 className="text-2xl font-semibold mb-2">Sale for {branch.branch_name}</h1>
      <p className="mb-6 text-sm text-gray-600">TIN: {branch.branch_tin}</p>
